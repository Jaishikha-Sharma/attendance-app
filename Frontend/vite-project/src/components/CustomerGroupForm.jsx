...
const CustomerGroupForm = ({ customerGroupLink = "", onSave }) => {
  const [link, setLink] = useState(customerGroupLink);
  const [editing, setEditing] = useState(!customerGroupLink);
  ...
  const handleSave = async () => {
    ...
    try {
      setSaving(true);
      await onSave(link);
      toast.success("Customer group link updated!");
      setEditing(false);
    } catch (err) {
      console.error("Failed to update:", err);
      toast.error("Failed to update customer group link.");
    } finally {
      setSaving(false);
    }
  };
